...
function RouteExec(route, req) {
	var urlRegExp = RouteToRe(route);
	
	console.log('re: ', urlRegExp);
	
	return urlRegExp.exec(req.url);
}

function RouteMatches(route, req) {
	var execed = RouteExec(route, req), good = execed !== null && execed !== undefined;
	if(route.method && good)
		good = (route.method == 'all' || route.method.toLowerCase() == req.method.toLowerCase());
...
exports.Router = new Class({
	...
	"getParams": function(route, url) {
		return RouteExec(route, {"url": url});
	}
});
